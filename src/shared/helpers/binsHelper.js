...
export const createBins = ({ start, end }) => {
...
export const getBinStartForPosition = (position) => {
    if (position < 1) {
        // this should never happen
        throw new Error('Feature coordinates can only be represented with positive integers');
    }
...
export const getBinEndForPosition = (position) => {
    return Math.ceil(position / BIN_SIZE) * BIN_SIZE;
};
export const createBinKey = ({ start, end }) => {
    return `${start}-${end}`;
};
